...
export function ProjectDetail({ projectId, onOpenAnnotate }: ProjectDetailProps) {
  ...
  // Below 1024 there is not room for both columns, so they become one screen at a
  // time: the list, or the document you picked from it. Nothing is hidden that
  // was not reachable — the back link returns to the list.
      ...
      {showDocs && (
        <section
          ...
          style={{
            flex: folded ? 1 : 'none',
            minWidth: 0,
            width: folded ? 'auto' : 'var(--ts-mid)',
            display: 'flex',
            flexDirection: 'column',
            borderRight: folded ? '0' : '2px solid var(--color-divider)',
          }}
        >
          <div
            style={{
